        ...
        $.get("/price", (data) => {
            $("#mining_share_calculate_table_body tr").each(function () {
                if ($(this).children("td:eq(2)").html() != "")
                    $(this).children("td:eq(3)").html(parseFloat($(this).children("td:eq(2)").html() * data).toFixed(2));
            });
        })
    }, 500);
...
function calculateShare(shareID) {
    ...
    $.get(`/api/mining/share/${shareID}/sharesResult`, (resultObject) => {
        for (const userID in resultObject.workedMilliseconds) {
            $(`#mining_share_calculate_table_row-${userID} td:eq(1)`).html(parseFloat(resultObject.workedMilliseconds[userID] / 1000 / 60 / 60).toFixed(2));
        }
        for (const userID in resultObject.amounts) {
            $(`#mining_share_calculate_table_row-${userID} td:eq(2)`).html(parseFloat(resultObject.amounts[userID]).toFixed(7));
        }
        $("#mining_share_calculate_table_foot td:eq(1)").html(parseFloat(resultObject.totalMilliseconds / 1000 / 60 / 60).toFixed(2));
    });
}